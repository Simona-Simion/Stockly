...
class _ProductoFormScreenState extends State<ProductoFormScreen> {
  ...
  // Nombre de la unidad seleccionada (null si no hay selección o lista vacía)
  String? get _unidadNombre {
    if (_unidadId == null) return null;
    final match = _unidades.where((u) => u['id'] == _unidadId).toList();
    return match.isNotEmpty ? (match.first['nombre'] as String?) : null;
  }

  // Devuelve el texto de ayuda según la unidad, o null si no aplica
  String? _ayudaStock(String nombre) {
    final n = nombre.toLowerCase();
    if (n == 'cl' || n == 'ml' || n == 'l') {
      return '💡 Introduce el stock en $nombre. '
          'Ej: 1 botella de 750ml = 750 en ml, o 75 en cl';
    }
    const contables = {'botella', 'lata', 'brik', 'unidad', 'kg', 'g'};
    if (contables.contains(n)) {
      return '💡 Introduce el número de $nombre que tienes en stock';
    }
    return null;
  }
