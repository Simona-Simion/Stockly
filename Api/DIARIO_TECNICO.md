...
## FASE 2 — Frontend Flutter PWA
**Fecha:** 08/03/2026

---

### Qué es Flutter y por qué se usa aquí

Flutter es un framework de Google para desarrollar aplicaciones con un único código base que compila a móvil (Android/iOS), web y escritorio. En Stockly se usa compilado como **PWA (Progressive Web App)**: una aplicación web que el móvil puede instalar como si fuera nativa, sin pasar por la tienda de aplicaciones.

**Por qué Flutter en lugar de una app web clásica:**
- Un único código sirve para web y para el futuro app móvil nativo
- El escáner de cámara (Fase 3) funciona con el mismo código en web y en Android
- Dart (el lenguaje de Flutter) tiene tipado estático, lo que reduce errores en tiempo de ejecución
...
├── utils/constants.dart   → URL de la API (configurable al compilar)
├── models/                → Clases que representan los datos del servidor
├── services/              → Llamadas HTTP a la API REST
├── providers/             → Gestión de estado con el patrón Provider
└── screens/               → Pantallas de la interfaz de usuario
```

Esta estructura sigue el patrón **MVC adaptado a Flutter**: los modelos son los datos, los servicios son la capa de acceso a datos (repositorio), los providers son el controlador, y las screens son la vista.

---

### Capa de modelos — fromJson

Cada clase modelo tiene un constructor factory `fromJson` que convierte el JSON que devuelve la API en un objeto Dart con tipos definidos:
...
**Por qué `(json['stockActual'] as num).toDouble()` y no `json['stockActual'] as double`:**
JSON no distingue entre int y double: `2` y `2.0` son dos representaciones válidas. Si el servidor devuelve `2` (int) y se hace cast a `double`, lanza una excepción en tiempo de ejecución. Casting a `num` primero y luego `.toDouble()` es seguro en ambos casos.

---

### Capa de servicios — cliente HTTP

`ApiService` centraliza toda la comunicación con el backend:
...
**Por qué centralizar en ApiService:**
- Si la API cambia su formato de respuesta, solo hay que modificar `_handleResponse` en un sitio
- Si se añade autenticación JWT (Fase 3), los headers se añaden aquí y afectan a todas las llamadas
- DRY (Don't Repeat Yourself): sin duplicar el tratamiento de errores en cada pantalla
...
dynamic _handleResponse(http.Response response) {
  if (response.statusCode >= 200 && response.statusCode < 300) {
    ...
    throw Exception(mensaje_del_servidor);
  }
}
```
- `utf8.decode(response.bodyBytes)` en lugar de `response.body` → evita problemas de codificación con caracteres como ñ, á, é
- La API envuelve las respuestas en `{ "data": ... }`. Si tiene esa clave, se extrae directamente

---

### Gestión de estado — patrón Provider

Flutter usa un árbol de widgets reactivo: cuando el estado cambia, los widgets que lo escuchan se reconstruyen automáticamente.

**¿Qué es un Provider?**
Un `ChangeNotifier` es una clase que notifica a sus oyentes cuando algo cambia. `Provider` es el paquete que integra esto con el árbol de widgets de Flutter.

```dart
// En main.dart: se registran los providers globalmente
MultiProvider(
  providers: [
...
```dart
// En la pantalla: escuchar cambios
Consumer<ProductoProvider>(
  builder: (context, provider, _) {
    if (provider.cargando) return CircularProgressIndicator();
    return ListView(children: provider.productos.map(...).toList());
  },
)
```

**Ciclo de vida de una carga:**
1. Pantalla llama a `provider.cargar()`
2. Provider pone `_cargando = true` y llama `notifyListeners()`
3. El `Consumer` reconstruye → muestra `CircularProgressIndicator`
4. El servicio HTTP completa → se guardan los datos, `_cargando = false`
5. `notifyListeners()` de nuevo → el `Consumer` reconstruye con la lista de datos

**Por qué Provider y no setState:**
`setState` solo funciona dentro del mismo widget. Si dos pantallas distintas necesitan el mismo dato (ej: la lista de productos aparece en Productos y en el formulario de Mermas), con `setState` habría que duplicar la carga. Con Provider, los datos se cargan una vez y cualquier pantalla los consume.
...
```
`IndexedStack` mantiene todas las pantallas en memoria aunque no sean visibles. La alternativa sería recrear cada pantalla al cambiar de pestaña, perdiendo el scroll y el estado de carga. Con `IndexedStack` el usuario puede volver a Productos sin que recargue desde cero.

#### ProductosScreen — Listado con búsqueda
- Búsqueda local en tiempo real (filtra la lista en memoria, sin llamar a la API en cada tecla)
- Los productos con `stockActual < stockMinimo` se marcan con borde rojo y texto de aviso
- `RefreshIndicator`: el usuario puede deslizar hacia abajo para recargar (patrón estándar en apps móviles)

#### ProductoDetalleScreen — Ficha del producto
- Muestra todos los campos del producto
- `LinearProgressIndicator` visual del nivel de stock: verde si está bien, rojo si está bajo mínimo
- Historial de movimientos filtrado solo para ese producto (llama a `GET /api/movimientos/producto/{id}`)

#### RegistrarVentaScreen — Escandallo visual
Cuando el camarero selecciona una receta y una cantidad, la pantalla muestra en tiempo real qué cantidad se descontará de cada ingrediente:
```
Cuba Libre × 2:
  Ron Bacardí    -0.100 L
  Coca-Cola      -0.400 L
Total: 12.00 €
```
Esta vista previa es puramente local (Dart multiplica cantidad × linea.cantidad). El descuento real lo hace el backend con `@Transactional` cuando se confirma la venta.

Si el backend rechaza la venta (stock insuficiente → HTTP 409), el error del servidor se muestra en un SnackBar rojo: *"Stock insuficiente: Ron Bacardí"*.

#### RegistrarMermaScreen — Registro de merma
El selector de producto muestra el stock actual junto al nombre:
```
Ron Bacardí (stock: 0.90)
```
Validación local antes de enviar: si el usuario introduce una cantidad mayor que el stock disponible, se muestra error en el propio campo sin necesidad de llamar a la API.

#### MovimientosScreen — Historial
Cada movimiento se colorea según su tipo:
- Azul → VENTA
- Naranja → MERMA
- Verde → ENTRADA
- Morado → AJUSTE
...
### Problema técnico resuelto — rutas con caracteres especiales en Windows

El compilador de shaders de Flutter (`impellerc.exe`) no puede escribir en rutas que contengan caracteres no-ASCII como `ñ` o `º`. El proyecto está en:
`C:\Users\monik\Escritorio\simona\ASIGNATURA_DAM-2º-año\...`

**Solución para compilar la PWA final:**
```bash
# Copiar a ruta sin caracteres especiales
xcopy /E /I stockly C:\stockly_build
cd C:\stockly_build
flutter build web --dart-define=API_URL=https://mi-servidor.com
```
El código fuente se sigue editando en la carpeta original. Solo se mueve para compilar.

---

### Preguntas que podría hacer el tribunal sobre Flutter

**¿Qué es una PWA?**
Progressive Web App: una web que el navegador puede instalar en el móvil como una app nativa. Tiene icono en el escritorio, puede funcionar sin conexión (con service worker) y usa la cámara igual que una app nativa. Flutter compila a PWA con `flutter build web`.

**¿Por qué Flutter y no React Native o una web clásica?**
Flutter permite usar el mismo código para web PWA ahora y para una app Android/iOS en el futuro sin reescribir nada. El escáner de cámara (Fase 3) funciona igual en web y en móvil con el paquete `mobile_scanner`.

**¿Qué es el patrón Provider?**
Es una implementación del patrón Observer para Flutter. Los `ChangeNotifier` mantienen el estado y notifican a los widgets cuando cambia. Los widgets se reconstruyen automáticamente. Es la alternativa recomendada por el equipo de Flutter para apps de tamaño medio.

**¿Qué es `IndexedStack` y por qué se usa?**
Widget que mantiene todos sus hijos en memoria pero solo muestra el que está seleccionado. Se usa en el `HomeScreen` para que al cambiar de pestaña no se pierda el scroll ni se vuelva a cargar la lista de la API.

**¿Por qué se valida el stock en el frontend si el backend también lo valida?**
El frontend valida para dar feedback inmediato al usuario sin esperar la respuesta del servidor (mejor UX). El backend valida porque nunca se debe confiar solo en el cliente: alguien podría llamar a la API directamente sin pasar por la app. Siempre ambas capas.

**¿Qué pasa si la API no está disponible?**
El `ApiService` lanza una excepción que el Provider captura y guarda en `_error`. Los widgets que usan `Consumer` muestran un mensaje de error con un botón "Reintentar". La app no se bloquea.
